import { BASE, TMDB_KEY, COLLAPS_API, COLLAPS_TOKEN, ALLOHA_TOKEN } from './config';

const apiCache = new Map();
const inflight = new Map();
const CATALOG_TTL = 900000;
...
export const api = async (path, ttl) => {
    ...
    if (cached) { revalidate(path); return cached.data; }
...
async function fetchAndCache(path) {
    ...
    const promise = (async () => {
        try {
            ...
            console.warn('TMDB API error:', err.message, path);
...
function revalidate(path) {
    ...
    const promise = (async () => {
        try {
            ...
            const data = await res.json();
            apiCache.set(path, { data, ts: Date.now() });
        } catch {} finally { inflight.delete(path); }
    })();
    inflight.set(path, promise);
}

export const getImdbId = async (tmdbId, type) => {
    try { const data = await api(`/${type}/${tmdbId}/external_ids`); return data?.imdb_id?.replace('tt', '') || null; } catch { return null; }
};

export const searchCollaps = async (imdbId) => {
    try { const res = await fetch(`${COLLAPS_API}/list?token=${COLLAPS_TOKEN}&imdb_id=${imdbId}`); const data = await res.json(); return data?.results?.[0] || null; } catch { return null; }
};
...
export const getMalId = async (title) => {
    try { const res = await fetch(`https://api.jikan.moe/v4/anime?q=${encodeURIComponent(title)}&limit=1`); const data = await res.json(); return data?.data?.[0]?.mal_id || null; } catch { return null; }
};
